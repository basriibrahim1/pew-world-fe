...
const ChatCompanyPages = () => {
  ...
  useEffect(() => {
    const fetchPekerja = async () => {
      try {
        const result = await axios.get(`${url}/hire/myhire`, {
          headers: {
  ...
  const fetchChats = async (hire_id) =>{
    try {
      ...
      res && setMessages(res)
      res && setReceiverId(res[0].receiver_id)
      res && setReceiverName(res[0].receiver_name)
  ...
  useEffect(()=>{
    ...
    if(submitted){
      const postChat = async () =>{
        try {
          const result = await axios.post(`${url}/hire/messages/${currentHireId}`, formChat, header)
          let res = result.data.data
              ...
              <div key={item.id}>
                <div className='d-flex ms-2 mt-4' onClick={()=>fetchChats(item.id)}>
                    ...
                    <div className='d-flex flex-column ms-3 justify-content-center'>
                        ...
                        <span style={{opacity:'0.5', marginTop:'-15px'}}>{item.position}</span>
                    </div>
                </div>
              </div>
            ))}
          ...
          {/* chat, hire_id, receiver_name, sender_name */}
            <div className='w-100 border-bottom border-4 border-light pb-2 d-flex justify-content-between'>
                <div className='d-flex'>
                    <h5 className='mt-4 ms-3 text-center'>{receiver_name ? receiver_name : ""}</h5>
                </div>
                <p className='mt-4' style={{opacity:'0.5'}}>{hirePos ? hirePos : "Select Chat to Begin"}</p>
              ...
              (
                <div key={index}>
                  <div className='me-4 mt-2 justify-content-end d-flex align-items-end'>
                      <p className='border-success border border-2 shadow-sm p-1 rounded me-3' style={{opacity:'0.7'}}>{item.chat}</p>
                  </div>
                </div>
              ) : item.receiver_id !== receiver_id ? (
                <div key={index}>
                <div className='ms-4 mt-2 justify-content-start d-flex align-items-end'>
                    <p className='border-primary border border-2 shadow-sm p-1 rounded me-3' style={{opacity:'0.7'}}>{item.chat}</p>
                </div>
              </div>
              ) : ("")
            )}
            ...
            <div className='d-flex justify-content-center align-items-center'>
                <input id="chat" name="chat" value={chat} onChange={(e)=> setChat(e.target.value)} className='w-100 py-3 border ps-4 m-4' style={{borderRadius:'25px'}} type="text" placeholder='type message...' />
                <button className=' me-5 px-3 py-2 border-0 bg-primary text-white' style={{borderRadius:'50%'}} onClick={handleSendMessage}><BsFillSendFill /></button>
            </div>
        </div>
        {/* end right */}
...
export default ChatCompanyPages